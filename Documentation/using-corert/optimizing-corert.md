...
* `<IlcMethodAlignment>Minimal</IlcMethodAlignment>`: sets method alignment to the minimal supported value. By default, the compiler tries to align method bodies to their optimal value that utilizes CPU caches the best. Setting alignment to a smaller value removes padding between methods, shrinking size of the outputs.
