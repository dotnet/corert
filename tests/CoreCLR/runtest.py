...
parser.add_argument("-test_filter_path", dest="test_filter_path", nargs='?', default=None)
...
def call_msbuild(coreclr_repo_location,
    ...
    command =   [dotnetcli_location,
                 "msbuild",
                 os.path.join(coreclr_repo_location, "tests", "CoreCLR", "runtest.proj"),
                 "/p:Runtests=true",
                 "/clp:showcommandline"]
...
def setup_args(args):
    ...
    coreclr_setup_args.verify(args,
                              "test_filter_path",
                              lambda arg: True,
                              "Error setting test_filter_path")
    ...
    if coreclr_setup_args.host_os != "Windows_NT" and not (is_same_os and is_same_arch and is_same_build_type):
        ...
        if not hasattr(coreclr_setup_args, "test_native_bin_location"):
    ...
    print("test_filter_path         :%s" % coreclr_setup_args.test_filter_path)
...
def build_test_wrappers(host_os, 
    ...
    if test_filter_path is None:
        ...
        os.environ["__Exclude"] = test_filter_path
...
def parse_test_results(host_os, arch, build_type, coreclr_repo_location, test_location):
    ...
    test_run_location = os.path.join(coreclr_repo_location, "bin", "Logs", "testResults.xml")
    ...
    if not os.path.isfile(test_run_location):
        ...
        if not found:
            print("Unable to find testResults.xml. This normally means the tests did not run.")
...
def do_setup(host_os, 
    ...
    if unprocessed_args.build_xunit_test_wrappers:
        ...
        build_test_wrappers(host_os, arch, build_type, coreclr_repo_location, test_location, args.altjit_arch, test_filter_path)
...
def main(args):
    ...
    host_os, arch, build_type, coreclr_repo_location, product_location, core_root, test_location, test_native_bin_location, test_filter_path = (
        ...
        coreclr_setup_args.test_filter_path
    )
