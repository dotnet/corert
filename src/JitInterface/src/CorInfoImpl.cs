...
#if READYTORUN
                            _compilation.NodeFactory.GetOrCreateReadyToRunHelper(helperId, field.OwningType, pResolvedToken.token)
#else
        ...
        private void getCallInfo(ref CORINFO_RESOLVED_TOKEN pResolvedToken, CORINFO_RESOLVED_TOKEN* pConstrainedResolvedToken, CORINFO_METHOD_STRUCT_* callerHandle, CORINFO_CALLINFO_FLAGS flags, ref CORINFO_CALL_INFO pResult)
        {
            ...
            else if (directCall)
            {
...
#if READYTORUN
                        _compilation.NodeFactory.GetOrCreateStringAllocatorMethodNode(targetMethod, pResolvedToken.token)
                ...
                else if (pResult.exactContextNeedsRuntimeLookup)
                {
                    ...
                    pResult.codePointerOrStubLookup.constLookup = CreateConstLookupToSymbol(
#if READYTORUN
                        _compilation.NodeFactory.GetOrCreateMethodEntrypointNode(targetMethod, pResolvedToken.token)
#else
                ...
                else
                {
                    ...
                    {
#if READYTORUN
                        instParam = _compilation.NodeFactory.GetOrCreateMethodGenericDictionary(concreteMethod, pResolvedToken.token);
#else
...
#if READYTORUN
                        _compilation.NodeFactory.GetOrCreateInterfaceDispatchCell(targetMethod, pResolvedToken.token
#else
