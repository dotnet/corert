...
namespace Internal.IL
{
    ...
    partial class ILImporter
    {
        ...
        private void GenerateProlog()
        {
            ...
            if (_method.RequiresInstArg()) // hidden param after shadow stack pointer
            {
                signatureIndex++;
            }
...
//                            int offset = GetTotalParameterOffset() + GetTotalLocalOffset();
        ...
        private LLVMValueRef LLVMFunctionForMethod(MethodDesc callee, StackEntry thisPointer, bool isCallVirt, TypeDesc constrainedType, MethodDesc runtimeDeterminedMethod, out bool hasHiddenParam)
        {
...
//                    PrintIntPtr(hiddenParam);
        ...
        private LLVMValueRef GetCallableGenericVirtualMethod(StackEntry objectPtr, MethodDesc method, MethodDesc callee, MethodDesc runtimeDeterminedMethod, bool hasHiddenParam)
        {
            Debug.Assert(!hasHiddenParam);  // TODO delete if never happens
            ...
            if (exactContextNeedsRuntimeLookup)
            {
                ...
                _dependencies.Add(node);
                slotRef = LLVM.BuildCall(_builder, helper, new LLVMValueRef[]
                {
                    GetShadowStack(),
                    GetGenericContext()
                }, "getHelper");
            ...
            // if
            var andRef = LLVM.BuildAnd(_builder, gvmPtrRef, LLVM.ConstInt(LLVM.Int32Type(), (ulong)FatFunctionPointerOffset, LLVMMisc.False), "andPtrOffset");
            ...
            //var gep = LLVM.BuildGEP(_builder, gvmPtrRef, new LLVMValueRef[] { LLVM.ConstInt(LLVM.Int32Type(), (ulong)2, LLVMMisc.False) }, "removeOffset");
            LLVM.BuildStore(_builder, minusOffsetPtr, functionPtrRef);
        ...
        private ExpressionEntry HandleCall(MethodDesc callee, MethodSignature signature, MethodDesc canonMethod, MethodSignature canonSignature, StackEntry[] argumentValues, MethodDesc runtimeDeterminedMethod, ILOpcode opcode = ILOpcode.call, TypeDesc constrainedType = null, LLVMValueRef calliTarget = default(LLVMValueRef), LLVMValueRef hiddenRef = default(LLVMValueRef), TypeDesc forcedReturnType = null)
        {
            ...
            {
                fn = LLVMFunctionForMethod(callee, signature.IsStatic ? null : argumentValues[0], opcode == ILOpcode.callvirt, constrainedType, runtimeDeterminedMethod, out hasHiddenParam);
            }
...
//            }
        ...
        private void ImportCalli(int token)
        {
            ...
            if (_method.ToString().Contains("StartupCodeHelpers"))
            {
                //top of stack will be a load, but we want the value before deref.
            }
...
//            PrintIntPtr(target);
            ...
            var andResRef = LLVM.BuildBinOp(_builder, LLVMOpcode.LLVMAnd, functionPtrAsInt, LLVM.ConstInt(LLVM.Int32Type(), (ulong)FatFunctionPointerOffset, LLVMMisc.False), "andFatCheck");
