...
namespace System.Globalization
{
    internal partial class FormatProvider
    {
        ...
        class DateTimeParse
        {
            ...
            internal static DateTime ParseExact(String s, String format, IFormatProvider provider, DateTimeStyles style, out TimeSpan offset)
            {
                DateTimeFormatInfo dtfi = provider == null ? DateTimeFormatInfo.CurrentInfo : DateTimeFormatInfo.GetInstance(provider);
            ...
            internal static bool TryParseExact(String s, String format, IFormatProvider provider, DateTimeStyles style, out DateTime result, out TimeSpan offset)
            {
                DateTimeFormatInfo dtfi = provider == null ? DateTimeFormatInfo.CurrentInfo : DateTimeFormatInfo.GetInstance(provider);
                result = DateTime.MinValue;
                offset = TimeSpan.Zero;
            ...
            private static bool TryParseExact(String s, String format, DateTimeFormatInfo dtfi, DateTimeStyles style, ref DateTimeResult result)
            {
                if (s == null)
                {
                ...
                if (format.Length == 0)
                {
                    result.SetFailure(ParseFailureKind.Format, SR.Format_BadFormatSpecifier, null);
                    return false;
                }
            ...
            internal static DateTime ParseExactMultiple(String s, String[] formats,
                                                    IFormatProvider provider, DateTimeStyles style, out TimeSpan offset)
            {
                DateTimeFormatInfo dtfi = provider == null ? DateTimeFormatInfo.CurrentInfo : DateTimeFormatInfo.GetInstance(provider);
                DateTimeResult result = new DateTimeResult();       // The buffer to store the parsing result.
                offset = TimeSpan.Zero;
            ...
            private static bool TryParseExactMultiple(String s, String[] formats,
            ...
            {
                internal DTT dtt;    // Store the token
            ...
            {
                //
                // <TODO> When we support more timezone, change this to return an instance of TimeZoneInfo.</TODO>
                //
                if (MatchWord(ref str, DateTimeFormatInfo.GMTName))
                {
                    return (true);
                }

                if (MatchWord(ref str, DateTimeFormatInfo.ZuluName))
                {
                    return (true);
                }
            ...
            **      FormatException if invalid timezone format is found.
            ...
            private static Boolean GetDayOfNNN(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
            {
                ...
                {
                    result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_BadDatePattern, dtfi.ShortDatePattern);
                    return false;
                }
            ...
            private static Boolean GetDayOfMN(ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
            {
                ...
                if (monthDayOrder == ORDER_DM)
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_BadDatePattern, dtfi.YearMonthPattern);
                        return false;
                    }
            ...
            internal static Boolean ProcessHebrewTerminalState(DS dps, ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
            {
                ...
                switch (dps)
                {
                    case DS.DX_MNN:
                        ...
                        {
                            result.SetFailure(ParseFailureKind.FormatBadDateTimeCalendar, SR.Format_BadDateTimeCalendar, null);
                            return false;
                        }
            ...
            internal static DateTime Parse(String s, IFormatProvider provider, DateTimeStyles styles)
            {
                DateTimeFormatInfo dtfi = provider == null ? DateTimeFormatInfo.CurrentInfo : DateTimeFormatInfo.GetInstance(provider);
                DateTimeResult result = new DateTimeResult();       // The buffer to store the parsing result.
                result.Init();
            ...
            internal static bool TryParse(String s, IFormatProvider provider, DateTimeStyles styles, out DateTime result, out TimeSpan offset)
            {
                DateTimeFormatInfo dtfi = provider == null ? DateTimeFormatInfo.CurrentInfo : DateTimeFormatInfo.GetInstance(provider);
                result = DateTime.MinValue;
                offset = TimeSpan.Zero;
            ...
            //
            private static bool TryParse(String s, DateTimeFormatInfo dtfi, DateTimeStyles styles, ref DateTimeResult result)
            {
                if (s == null)
                {
                    result.SetFailure(ParseFailureKind.ArgumentNull, SR.ArgumentNull_String, null, "s");
                ...
                //
                //<TODO>@ToDo: We need a more efficient way of doing this.
                ...
                do
                {
                    ...
                    if (dtok.dtt != DTT.Unk)
                    {
                        ...
                        if (raw.hasSameDateAndTimeSeparators)
                        {
                            ...
                            if (DateParsingStates(dps, dtok.dtt) == DS.ERROR || atEnd)
                            {
                                switch (dtok.dtt)
                                {
                ...
                if (raw.dayOfWeek != -1)
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.Format, SR.Format_BadDayOfWeek, null);
                        return false;
                    }
                }
            ...
            private static Boolean DetermineTimeZoneAdjustments(ref DateTimeResult result, DateTimeStyles styles, Boolean bTimeOnly)
            {
                ...
                else
                {
                    ...
                    if (offsetTicks < InternalGloablizationHelper.MinOffset || offsetTicks > InternalGloablizationHelper.MaxOffset)
                    {
                        result.SetFailure(ParseFailureKind.Format, SR.Format_OffsetOutOfRange, null);
                        return false;
                    }
                }
                ...
                if ((result.flags & ParseFlags.TimeZoneUsed) == 0)
                {
                    ...
                    if ((styles & DateTimeStyles.AssumeLocal) != 0)
                    {
                        ...
                        {
                            result.flags |= ParseFlags.TimeZoneUsed;
                            result.timeZoneOffset = TimeZoneInfo.Local.GetUtcOffset(result.parsedDate);
                        }
                        else
                        {
            ...
            **  Only support GMT timezone.
                ...
                ref DateTimeResult result)
            {
                ...
                if (parseInfo.fUseTwoDigitYear && ((dtfi.FormatFlags & DateTimeFormatFlags.UseHebrewRule) == 0))
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.Format, SR.Format_BadDateTime, e);
                        return false;
                    }
                }
            ...
            private static Boolean DateTimeOffsetTimeZonePostProcessing(ref DateTimeResult result, DateTimeStyles styles)
            {
                ...
                if (utcTicks < InternalGloablizationHelper.MinTicks || utcTicks > InternalGloablizationHelper.MaxTicks)
                {
                    result.SetFailure(ParseFailureKind.Format, SR.Format_UTCOutOfRange, null);
                    return false;
                }
            ...
            private static Boolean AdjustTimeZoneToUniversal(ref DateTimeResult result)
            {
                ...
                if (resultTicks < InternalGloablizationHelper.MinTicks || resultTicks > InternalGloablizationHelper.MaxTicks)
                {
                    result.SetFailure(ParseFailureKind.Format, SR.Format_DateOutOfRange, null);
                    return false;
                }
            ...
            **Exceptions:
            ...
            private static bool CheckNewValue(ref int currentValue, int newValue, char patternChar, ref DateTimeResult result)
            {
                ...
                else
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_RepeatDateTimePattern, patternChar);
                        return (false);
                    }
                }
                return (true);
            ...
            private static bool CheckDefaultDateTime(ref DateTimeResult result, ref Calendar cal, DateTimeStyles styles)
            {
                if ((result.flags & ParseFlags.CaptureOffset) != 0)
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.Format, SR.Format_MissingIncompleteDate, null);
                        return false;
                    }
                }
                ...
                switch (ch)
                {
                    ...
                    case 'F':
                        ...
                        if (tokenLen <= DateTimeFormat.MaxSecondsFractionDigits)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_RepeatDateTimePattern, ch);
                                    return (false);
                                }
                            }
                        }
                    ...
                    case 'z':
                        ...
                        tokenLen = format.GetRepeatCount();
                        {
                            ...
                            {
                                result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_RepeatDateTimePattern, 'z');
                                return (false);
                            }
                    ...
                    case 'K':
                        ...
                        if (str.Match('Z'))
                        {
                            if ((result.flags & ParseFlags.TimeZoneUsed) != 0 && result.timeZoneOffset != TimeSpan.Zero)
                            {
                                result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_RepeatDateTimePattern, 'K');
                                return (false);
                            }
                    ...
                    case '\'':
                        ...
                        {
                            result.SetFailure(ParseFailureKind.FormatWithParameter, SR.Format_BadQuote, ch);
                            return (false);
                        }
                        ...
                        if ((result.flags & ParseFlags.CaptureOffset) != 0)
                        {
                            if ((result.flags & ParseFlags.Rfc1123Pattern) != 0 && quotedStr == DateTimeFormatInfo.GMTName)
                            {
                                result.flags |= ParseFlags.TimeZoneUsed;
                                result.timeZoneOffset = TimeSpan.Zero;
                            }
                            else if ((result.flags & ParseFlags.UtcSortPattern) != 0 && quotedStr == DateTimeFormatInfo.ZuluName)
                            {
                                result.flags |= ParseFlags.TimeZoneUsed;
                                result.timeZoneOffset = TimeSpan.Zero;
                            }
                    ...
                    default:
                        ...
                        {
                            if (format.MatchSpecifiedWord(DateTimeFormatInfo.GMTName))
                            {
                                format.Index += (DateTimeFormatInfo.GMTName.Length - 1);
                                // Found GMT string in format.  This means the DateTime string
                                // is in GMT timezone.
                                result.flags |= ParseFlags.TimeZoneUsed;
                                result.timeZoneOffset = TimeSpan.Zero;
                                if (!str.Match(DateTimeFormatInfo.GMTName))
                                {
                                    result.SetFailure(ParseFailureKind.Format, SR.Format_BadDateTime, null);
                                    return false;
                                }
            ...
            private static Exception GetDateTimeParseException(ref DateTimeResult result)
            {
                switch (result.failure)
                {
                    case ParseFailureKind.ArgumentNull:
                        return new ArgumentNullException(result.failureArgumentName, result.failureMessageID);
                    case ParseFailureKind.Format:
                        return new FormatException(result.failureMessageID);
                    case ParseFailureKind.FormatWithParameter:
                        return new FormatException(SR.Format(result.failureMessageID, result.failureMessageFormatArgument));
                    case ParseFailureKind.FormatBadDateTimeCalendar:
                        return new FormatException(SR.Format(result.failureMessageID, result.calendar));
                    default:
                        return null;
                }
            }
        ...
        struct __DTString
        {
            ...
            internal __DTString(String str, DateTimeFormatInfo dtfi)
            {
                ...
                if (dtfi != null)
                {
                    _info = dtfi.CompareInfo;
                    _checkDigitToken = ((dtfi.FormatFlags & DateTimeFormatFlags.UseDigitPrefixInTokens) != 0);
                }
                else
                {
                    _info = CultureInfo.CurrentCulture.CompareInfo;
                    _checkDigitToken = false;
                }
            }
            ...
            internal bool MatchSpecifiedWord(String target, int endIndex)
            {
                ...
                return (_info.Compare(Value, Index, count, target, 0, count, CompareOptions.IgnoreCase) == 0);
            }

            private static Char[] s_whiteSpaceChecks = new Char[] { ' ', '\u00A0' };
            ...
            internal bool MatchSpecifiedWords(String target, bool checkWordBoundary, ref int matchLength)
            {
                ...
                if (matchLength > valueRemaining || _info.Compare(Value, Index, matchLength, target, 0, matchLength, CompareOptions.IgnoreCase) != 0)
                {
                    // Check word by word
                    int targetPosition = 0;                 // Where we are in the target string
                    int thisPosition = Index;         // Where we are in this string
                    int wsIndex = target.IndexOfAny(s_whiteSpaceChecks, targetPosition);
                    if (wsIndex == -1)
                    {
                        return false;
                    }
                    do
                    {
                        ...
                        else
                        {
                            ...
                            if (_info.Compare(Value, thisPosition, segmentLength, target, targetPosition, segmentLength, CompareOptions.IgnoreCase) != 0)
                            {
                                return false;
                            }
                    ...
                    } while ((wsIndex = target.IndexOfAny(s_whiteSpaceChecks, targetPosition)) >= 0);
            ...
            internal bool Match(String str)
            {
                ...
                if (_info.Compare(Value, Index, str.Length, str, 0, str.Length, CompareOptions.Ordinal) == 0)
                {
        ...
        unsafe struct DateTimeRawInfo
        {
            ...
            //
            // <TODO>when we support more timezone names, change this to be
            // type of TimeZoneInfo.</TODO>
            //
