...
namespace System.Globalization
{
    ...
    {
        private static class TimeSpanParse
        {
            ...
            private static readonly TimeSpanToken s_zero = new TimeSpanToken(0);
            ...
            private struct TimeSpanTokenizer
            {
                ...
                internal TimeSpanToken GetNextToken()
                {
                    ...
                    else
                    {
                        ...
                        tok.sep = _value.Substring(startIndex, length);
                        return tok;
                    }
                }
            ...
            private struct TimeSpanRawInfo
            {
                ...
                internal Boolean ProcessToken(ref TimeSpanToken tok, ref TimeSpanResult result)
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.Overflow, SR.Overflow_TimeSpanElementTooLarge, null);
                        return false;
                    }
                    if (tok.ttt != TTT.Sep && tok.ttt != TTT.Num)
                    {
                        // Some unknown token or a repeat token type in the input
                        result.SetFailure(ParseFailureKind.Format, SR.Format_BadTimeSpan, null);
                        return false;
                    }
            ...
            private struct TimeSpanResult
            {
                ...
                internal string m_failureMessageFormat;
                ...
                {
                    m_failure = failure;
                    m_failureMessageFormat = failureMessageFormat;
                ...
                internal Exception GetTimeSpanParseException()
                {
                    switch (m_failure)
                    {
                        case ParseFailureKind.ArgumentNull:
                            return new ArgumentNullException(m_failureArgumentName, m_failureMessageFormat);

                        case ParseFailureKind.FormatWithParameter:
                            return new FormatException(SR.Format(m_failureMessageFormat, m_failureMessageFormatArgument));

                        case ParseFailureKind.Format:
                            return new FormatException(m_failureMessageFormat);

                        case ParseFailureKind.Overflow:
                            return new OverflowException(m_failureMessageFormat);
            ...
            private static Boolean ProcessTerminal_DHMSF(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
            {
                ...
                if (match)
                {
                    ...
                    {
                        result.SetFailure(ParseFailureKind.Overflow, SR.Overflow_TimeSpanElementTooLarge);
                        return false;
                    }
                    if (!positive)
                    {
            ...
            private static Boolean ProcessTerminal_HMS_F_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
            {
                ...
                if (inv)
                {
                    ...
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, s_zero, raw.numbers[0], raw.numbers[1], raw.numbers[2], raw.numbers[3], out ticks);
                        overflow = overflow || !match;
                    }
                    if (!match && raw.FullDHMSMatch(raw.PositiveInvariant))
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, raw.numbers[0], raw.numbers[1], raw.numbers[2], raw.numbers[3], s_zero, out ticks);
                        overflow = overflow || !match;
                    }
                    if (!match && raw.FullAppCompatMatch(raw.PositiveInvariant))
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, raw.numbers[0], raw.numbers[1], raw.numbers[2], s_zero, raw.numbers[3], out ticks);
                        overflow = overflow || !match;
                    }
            ...
            private static Boolean ProcessTerminal_HM_S_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
            {
                ...
                if (inv)
                {
                    ...
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, s_zero, raw.numbers[0], raw.numbers[1], raw.numbers[2], s_zero, out ticks);
                        overflow = overflow || !match;
                    }
                    if (!match && raw.FullDHMMatch(raw.PositiveInvariant))
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, raw.numbers[0], raw.numbers[1], raw.numbers[2], s_zero, s_zero, out ticks);
                        overflow = overflow || !match;
                    }
                    if (!match && raw.PartialAppCompatMatch(raw.PositiveInvariant))
                    {
                        positive = true;
                        match = TryTimeToTicks(positive, s_zero, raw.numbers[0], raw.numbers[1], s_zero, raw.numbers[2], out ticks);
                        overflow = overflow || !match;
                    }
            ...
            private static Boolean ProcessTerminal_HM(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
            {
                ...
                if (match)
                {
                    if (!TryTimeToTicks(positive, s_zero, raw.numbers[0], raw.numbers[1], s_zero, s_zero, out ticks))
                    {
                        result.SetFailure(ParseFailureKind.Overflow, SR.Overflow_TimeSpanElementTooLarge);
                        return false;
                    }
            ...
            private static Boolean ProcessTerminal_D(ref TimeSpanRawInfo raw, TimeSpanStandardStyles style, ref TimeSpanResult result)
            {
                ...
                if (match)
                {
                    if (!TryTimeToTicks(positive, raw.numbers[0], s_zero, s_zero, s_zero, s_zero, out ticks))
                    {
                        result.SetFailure(ParseFailureKind.Overflow, SR.Overflow_TimeSpanElementTooLarge);
                        return false;
                    }
            ...
            private static Boolean TryParseExactTimeSpan(String input, String format, IFormatProvider formatProvider, TimeSpanStyles styles, ref TimeSpanResult result)
            {
                ...
                if (format.Length == 0)
                {
                    result.SetFailure(ParseFailureKind.Format, SR.Format_BadFormatSpecifier);
                    return false;
                }
                ...
                while (i < format.Length)
                {
                    ...
                    switch (ch)
                    {
                        case 'h':
                            ...
                            {
                                result.SetFailure(ParseFailureKind.Format, SR.Format_InvalidString);
                                return false;
                            }
                            seenHH = true;
                            break;
