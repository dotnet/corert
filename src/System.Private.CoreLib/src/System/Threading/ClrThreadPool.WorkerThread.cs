        ...
        private static class WorkerThread
        {
            ...
            private static void WorkerThreadStart()
            {
                ...
                while (true)
                {
                    ...
                    {
                        Volatile.Write(ref s_threadPoolInstance._separated.lastDequeueTime, Environment.TickCount);
                        if (TakeActiveRequest())
                        {
                            if (ThreadPoolWorkQueue.Dispatch())
                            {
                                ...
                                ThreadCounts currentCounts = ThreadCounts.VolatileReadCounts(ref s_threadPoolInstance._separated.counts);
                                while (true)
                                {
                                    ThreadCounts newCounts = currentCounts;
                                    newCounts.numProcessingWork--;
                                    ThreadCounts oldCounts = ThreadCounts.CompareExchangeCounts(ref s_threadPoolInstance._separated.counts, newCounts, currentCounts);
                                ...
                                if (s_threadPoolInstance._numRequestedWorkers > 0)
                                {
                    ...
                    ThreadCounts counts = ThreadCounts.VolatileReadCounts(ref s_threadPoolInstance._separated.counts);
                    while (true)
                    {
                        ...
                        newCounts.numThreadsGoal = Math.Max(s_threadPoolInstance._minThreads, Math.Min(newCounts.numExistingThreads, newCounts.numThreadsGoal));
                        ThreadCounts oldCounts = ThreadCounts.CompareExchangeCounts(ref s_threadPoolInstance._separated.counts, newCounts, counts);
                        if (oldCounts == counts)
                        {
            ...
            {
                int count = s_threadPoolInstance._numRequestedWorkers;
                while (count > 0)
                {
                    int prevCount = Interlocked.CompareExchange(ref s_threadPoolInstance._numRequestedWorkers, count - 1, count);
                    if (prevCount == count)
                    {
                        return true;
                    }
